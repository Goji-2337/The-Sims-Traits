...
namespace SimsTraits
{
    ...
    public static class Pawn_InteractionsTracker_InteractionsTrackerTick_Patch
    {
        ...
        public static bool Prefix(Pawn_InteractionsTracker __instance)
        {
            ...
            {
                InteractionsTrackerTick(__instance);
                return false;
            }
            return true;
        }

        public static void InteractionsTrackerTick(Pawn_InteractionsTracker __instance)
        {
            ...
            if (!__instance.wantsRandomInteract)
            {
                if (Find.TickManager.TicksGame > __instance.lastInteractionTime + (320 / InteractionRate) 
                    && __instance.pawn.IsHashIntervalTick(60))
                {
                    int num = 0;
                    switch (currentSocialMode)
                    {
            ...
            else if (__instance.pawn.IsHashIntervalTick(91) && __instance.TryInteractRandomly())
            {
                __instance.wantsRandomInteract = false;
            }
        }
